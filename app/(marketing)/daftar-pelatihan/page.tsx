...
function DaftarPelatihanContent() {
  ...
  const details = activeProduct ? (() => {
    // If database product has custom details JSON, use it first!
    if (activeProduct.dbDetails) {
      ...
      return {
        ...
        whyInteresting: dbD.whyInteresting || ["Modul belajar praktis", "Mentor berpengalaman", "Sertifikat resmi", "Akses jaringan industri"],
        targetMarket: dbD.targetMarket || ["?? Mahasiswa & alumni", "?? Profesional muda", "?? Wirausaha rintisan"],
        disclaimer: dbD.disclaimer || "Jadwal dan materi kelas diatur setelah konfirmasi pendaftaran selesai."
      };
    }

    const tdKeyActive = Object.keys(trainingDetails).find(k => k.toLowerCase() === activeProduct.name.toLowerCase());
    return tdKeyActive ? trainingDetails[tdKeyActive] : {
