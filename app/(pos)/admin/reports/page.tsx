    ...
    <div className="p-8 lg:p-12 space-y-10 max-w-[1600px] mx-auto min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          { label: "Pendaftaran Menunggu", value: pendingBookings.length || 0, icon: CalendarDays, color: pendingBookings.length > 0 ? "text-sky-600 animate-pulse" : "text-sky-600", bg: "bg-sky-50" }
        ].map((stat, idx) => (
          <motion.div
            key={stat.label}
      ...
      {activeTab === "finance" ? (
                  ...
                  <AnimatePresence mode="popLayout">
                    {loading ? (
                      ...
                      >
                        ...
                        <td className="px-8 py-6">
                          <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-1 rounded-md ${t.type === "BOOKING" ? "bg-indigo-50 text-indigo-600" : "bg-sky-50 text-sky-600"}`}>
                            {t.type}
                          </span>
                        ...
                        <td className="px-8 py-6">
                          <div className="flex justify-center">
                            <div className={`flex items-center gap-2 px-3 py-1 rounded-lg border text-[8px] font-black uppercase tracking-widest ${t.status === "SUCCESS" || t.status === "COMPLETED" || t.status === "CONFIRMED" ? "bg-emerald-50 border-emerald-100 text-emerald-600" : "bg-sky-50 border-sky-100 text-sky-600"}`}>
                              <div className={`w-1 h-1 rounded-full ${t.status === "SUCCESS" || t.status === "COMPLETED" || t.status === "CONFIRMED" ? "bg-emerald-600" : "bg-sky-600"}`} />
                              {t.status}
                            </div>
                          </div>
                        </td>
        ...
        <>
          ...
          {pendingBookings.length > 0 && (
            <div className="bg-sky-50/20 border border-sky-200/50 rounded-3xl p-6 md:p-8 space-y-6">
              <div className="flex items-center gap-3 border-b border-sky-200/30 pb-4">
                <div className="w-10 h-10 rounded-xl bg-sky-500 text-white flex items-center justify-center animate-pulse">
                  <CalendarDays size={20} />
                </div>
                <div>
                  <h3 className="text-sm font-black text-sky-800 uppercase tracking-wider">Antrean Konfirmasi Kelas</h3>
                  <p className="text-[10px] text-sky-700/60 font-semibold uppercase tracking-widest mt-0.5">{pendingBookings.length} Pendaftaran Menunggu Verifikasi</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {pendingBookings.map((booking) => (
                  <motion.div
                    key={booking.id}
                    layout
                    className="bg-white rounded-2xl p-5 border border-sky-200/30 shadow-sm flex flex-col justify-between space-y-4"
                  >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-3 space-y-1.5 text-xs text-gray-500 font-semibold">
                          ...
                          <p className="flex items-center gap-1.5">
                            ...
                            Bayar: <span className="font-bold text-sky-700">{booking.payment_method.toUpperCase()}</span> (Rp {booking.final_price.toLocaleString("id-ID")})
                          </p>
                        </div>
                      </div>
                    </div>
                      ...
                      return (
                        <React.Fragment key={c.phone || idx}>
                          <motion.tr
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            className={`group transition-colors ${hasPending ? "bg-sky-50/40 hover:bg-sky-50/70" : "hover:bg-[#f0f7ff]/30"}`}
                          >
                            <td className="px-8 py-6">
                              <div className="flex items-center gap-4">
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs relative ${hasPending ? "bg-sky-100 text-sky-800" : "bg-[#1e293b]/5 text-[#1e293b]"}`}>
                            ...
                            <td className="px-8 py-6">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {hasPending && (
                                  <span className="text-xs font-bold uppercase tracking-widest px-3 py-1.5 bg-sky-100 text-sky-700 rounded-md animate-pulse">
                          ...
                          <AnimatePresence>
                            {isExpanded && hasPending && (
                              <motion.tr
                                ...
                                exit={{ opacity: 0 }}
                              >
                                <td colSpan={5} className="px-8 pb-6 pt-0 bg-sky-50/20">
                                  <div className="border border-sky-200/40 rounded-2xl overflow-hidden bg-white shadow-sm divide-y divide-sky-50">
