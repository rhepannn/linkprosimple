...
export default function CustomersAndRevenue() {
  ...
  const exportToExcel = async () => {
    ...
    XLSX.writeFile(wb, `LinkProductive_${view === "customers" ? "Pelanggan" : "Pendapatan"}_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  return (
    <div className="p-8 lg:p-12 space-y-10 max-w-[1600px] mx-auto min-h-screen">
      {/* ── Header Section ── */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-[#111e38]/5 pb-10">
        <div className="space-y-2">
          <p className="text-xs font-black text-gold uppercase tracking-[0.3em]">Analytics Engine</p>
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-near-black flex items-center justify-center text-white shadow-xl shadow-near-black/20">
              <Activity size={24} />
            </div>
            <h1 className="text-4xl font-black text-near-black tracking-tight" style={{ fontFamily: "var(--font-playfair)" }}>Data Pelanggan</h1>
          </div>
          <p className="text-sm text-gray-500 font-medium max-w-md">Analisis metrik perilaku pelanggan dan retensi pendapatan platform.</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex bg-gray-100 p-1 rounded-xl">
            <button
              onClick={() => setView("customers")}
              className={`px-6 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${view === "customers" ? "bg-white text-near-black shadow-sm" : "text-gray-400 hover:text-near-black"}`}
            >
              Pelanggan
            </button>
            <button
              onClick={() => setView("revenue")}
              className={`px-6 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${view === "revenue" ? "bg-white text-near-black shadow-sm" : "text-gray-400 hover:text-near-black"}`}
            >
              Pendapatan
            </button>
          </div>
          <button
            ...
            className="flex items-center gap-3 px-8 py-4 bg-near-black !text-white rounded-xl text-xs font-black uppercase tracking-[0.2em] shadow-xl shadow-near-black/20 hover:scale-[1.02] active:scale-[0.98] transition-all"
          >
            <Download size={16} />
            Ekspor
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          <motion.div
            ...
            className="p-7 bg-white rounded-2xl border border-white shadow-sm flex flex-col justify-between group hover:shadow-xl hover:shadow-near-black/5 transition-all duration-500"
          >
            ...
            <div>
              ...
              <div className="flex items-end gap-2">
                <span className="text-4xl font-black text-near-black tracking-tighter">{stat.value}</span>
              </div>
            </div>
          </motion.div>
        ))}
