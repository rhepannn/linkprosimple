...
export function KegiatansSection({ settings = {} }: { settings?: Record<string, string> }) {
  ...
  return (
    <section
      id="kegiatan"
      className="relative bg-[#f0f7ff] py-20 lg:py-28 overflow-hidden w-full max-w-full border-b border-slate-100"
    >
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16 space-y-4">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-[#004aad] tracking-tight">
            {kegiatan_title}{" "}
            {kegiatan_highlight && <span className="text-[#004aad]">{kegiatan_highlight}</span>}
          </h2>
          <p className="text-sky-600 text-sm font-medium">{kegiatan_desc}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {categories.map((cat, i) => (
            ...
            >
              ...
              <div className="relative aspect-[16/10] bg-slate-100 border-b border-slate-100 flex flex-col items-center justify-center p-4 text-sky-500">
              ...
              <div className="p-6 flex flex-col flex-grow justify-between">
                <div className="space-y-2 mb-6">
                  <h3 className="text-lg font-bold text-[#004aad] group-hover:text-[#004aad]">{cat.title}</h3>
                  <p className="text-xs text-sky-700 leading-relaxed font-medium">{cat.desc}</p>
                </div>
