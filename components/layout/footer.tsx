...
const NAV_LINKS = [
  ...
  { label: "Program Pelatihan", href: "/packages" },
  { label: "Tentang Kami", href: "/#about" },
  { label: "Kontak", href: "/#contact" },
];

const muted = { color: "#7c95c4" } as const;
const normal = { color: "#2b3e63" } as const;
const bright = { color: "#111e38" } as const;
...
export function Footer() {
  ...
  return (
    <footer style={{ backgroundColor: "#ebf1fa", borderTop: "1px solid rgba(0, 74, 173, 0.1)" }} role="contentinfo">
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pt-10 pb-8 sm:pt-14 sm:pb-10">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-8">
          ...
          <div className="col-span-2 lg:col-span-1">
            <Link href="/" aria-label="Link Productive — Beranda"
              className="block mb-6">
              <Logo height={42} textColor="#004AAD" />
            </Link>
            ...
            <div className="flex items-center gap-2">
              <a href={site.contact.instagram} target="_blank" rel="noopener noreferrer"
                ...
                style={{ backgroundColor: "rgba(0, 74, 173, 0.05)", border: "1px solid rgba(0, 74, 173, 0.12)" }}>
                <InstagramIcon size={13} fill="#004AAD" />
              </a>
              <a href={site.contact.tiktok} target="_blank" rel="noopener noreferrer"
                aria-label="TikTok Link Productive"
                className="flex items-center justify-center w-7 h-7 rounded-md transition-all duration-200"
                style={{ backgroundColor: "rgba(0, 74, 173, 0.05)", border: "1px solid rgba(0, 74, 173, 0.12)" }}>
                <TikTokIcon size={13} fill="#004AAD" />
              </a>
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {NAV_LINKS.map((link) => (
                <li key={link.href}>
                  <Link href={link.href}
                    className="text-xs hover:text-[#004AAD] transition-colors duration-200"
                    style={normal}>
                    {link.label}
                  </Link>
                </li>
            ...
            <ul className="space-y-2.5">
              ...
              <li className="flex items-center gap-2">
                ...
                <a href={`mailto:${site.contact.email}`}
                  className="text-xs hover:text-[#004AAD] transition-colors duration-200 break-all"
                  style={normal}>
                  {site.contact.email}
                </a>
              </li>
          ...
          <div>
            ...
            <div className="mt-3 flex items-center gap-1.5">
              ...
              <span className="text-[10px] font-medium" style={{ color: "#1D9E75" }}>Aktif</span>
            </div>
          </div>
      ...
      {/* ── Copyright ── */}
      <div style={{ borderTop: "1px solid rgba(0, 74, 173, 0.1)" }}>
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 py-3.5">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
            <p className="text-[10px]" style={{ color: "#7c95c4" }}>
              © {currentYear} {site.name}. All rights reserved.
            </p>
            <p className="text-[10px] hidden sm:block" style={{ color: "#7c95c4" }}>
              Program Pelatihan & Inovasi Sosial · {site.contact.address}
            </p>
          </div>
        </div>
      </div>
