  ...
  return (
    <div className={`space-y-12 relative ${!hideHeader ? 'min-h-screen bg-[#FAFAF8] p-6 lg:p-12' : ''}`}>
      {!hideHeader && (
        <>
          {/* ── Background Protocol ── */}
          <div className="absolute top-0 left-0 w-full h-[600px] bg-gradient-to-b from-[#1e293b]/5 to-transparent pointer-events-none" />
          <div className="absolute -top-24 -right-24 w-96 h-96 bg-[#1e293b]/3 blur-[120px] rounded-full pointer-events-none" />
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-10 mb-16 relative z-10">
            ...
            >
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-[22px] bg-[#1e293b] flex items-center justify-center text-white shadow-2xl shadow-[#1e293b]/20">
                  <Package size={28} />
                </div>
                <div>
                  <h1 className="text-4xl font-bold text-[#1e293b] tracking-tight" style={{ fontFamily: "var(--font-outfit)" }}>
                    Manajemen Program Pelatihan
                  </h1>
                  <p className="text-[10px] font-black uppercase tracking-[0.4em] text-[#1e293b]/40">Katalog Kelas Pelatihan & Layanan Kemitraan</p>
                </div>
              </div>
            </motion.div>
            ...
            >
              <button
                ...
                className="p-4 bg-white border border-[#1e293b]/5 rounded-[20px] text-[#1e293b]/40 hover:text-[#1e293b] hover:shadow-xl transition-all active:scale-90"
              >
              ...
              <button
                ...
                className="flex items-center gap-4 px-8 py-5 bg-[#1e293b] !text-white rounded-[24px] text-[11px] font-black uppercase tracking-[0.3em] shadow-2xl shadow-[#1e293b]/20 hover:scale-105 active:scale-95 transition-all select-none"
              >
                <Plus size={20} />
                Tambah Produk Baru
              </button>
              ...
              <button
                onClick={async () => {
                  setConfirmDialog({
                    ...
                    onConfirm: async () => {
                      setConfirmDialog(null);
                      const { seedBrandProducts } = await import("@/app/actions/products");
                      const res = await seedBrandProducts();
                      if (res.success) {
      ...
      <div className="flex flex-col xl:flex-row gap-8 relative z-10">
        <div className="relative flex-1 group">
          <Search className="absolute left-7 top-1/2 -translate-y-1/2 text-[#1e293b]/20 group-focus-within:text-[#1e293b] transition-colors" size={20} />
          <input
            type="text"
            placeholder="Cari paket atau SKU..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full pl-16 pr-8 py-6 bg-white border border-[#1e293b]/5 rounded-2xl text-sm font-medium focus:outline-none focus:ring-4 focus:ring-[#1e293b]/2 shadow-sm transition-all"
          />
        </div>
        ...
        <div className="flex items-center gap-4">
          {hideHeader && (
            <button
              ...
              className="flex items-center gap-3 px-8 py-4.5 bg-[#1e293b] !text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl shadow-[#1e293b]/10 hover:scale-[1.02] active:scale-[0.98] transition-all whitespace-nowrap"
            >
              <Plus size={16} />
              Tambah Paket
            </button>
          ...
          {loading ? (
            ...
            filteredProducts.map((p) => (
              <motion.div
                ...
                className="bg-white p-6 rounded-3xl border border-white shadow-sm hover:shadow-xl hover:shadow-[#1e293b]/5 transition-all duration-500 group relative overflow-hidden flex flex-col"
              >
                {/* Status Badge */}
                <div className={`absolute top-6 right-6 px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest border z-20 ${p.isActive ? 'bg-green-50 text-green-600 border-green-100' : 'bg-gray-50 text-gray-400 border-gray-100'
                  }`}>
                  {p.isActive ? 'Active' : 'Inactive'}
                </div>

                <div className="space-y-6 flex-1">
                  <div className="w-full aspect-[4/3] bg-[#f0f7ff] rounded-2xl flex items-center justify-center border border-[#1e293b]/2 overflow-hidden relative">
                    {p.image ? (
                      <img src={p.image} alt={p.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                    ) : (
                      <div className="flex flex-col items-center gap-3 text-[#1e293b]/10">
                  ...
                  <div className="space-y-4 px-2">
                    ...
                    <h3 className="text-lg font-bold text-[#1e293b] leading-tight line-clamp-1 group-hover:text-[#0ea5e9] transition-colors">{p.name}</h3>
                ...
                <div className="mt-8 pt-6 border-t border-[#f0f7ff] flex items-center justify-between">
                  <div className="flex flex-col">
                    <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Inventory</span>
                    <span className="text-xs font-bold text-[#1e293b]">{p.stock} Units</span>
                  </div>
                  <div className="flex gap-2">
                    <button
                      ...
                      className="w-10 h-10 flex items-center justify-center bg-[#f0f7ff] text-[#1e293b]/60 hover:bg-[#1e293b] hover:text-white rounded-xl transition-all shadow-sm"
                    >
                      <Edit size={16} />
                    </button>
                    <button
            ...
            <div className="col-span-full py-32 text-center space-y-8">
              <div className="w-32 h-32 bg-white rounded-[40px] flex items-center justify-center text-[#1e293b]/5 mx-auto border border-[#1e293b]/2 shadow-2xl shadow-[#1e293b]/5">
                <Search size={64} strokeWidth={1} />
              </div>
              <div>
                <h3 className="text-2xl font-bold text-[#1e293b]">Produk tidak ditemukan</h3>
                <p className="text-sm text-[#1e293b]/40 mt-2 tracking-widest uppercase font-black text-[10px]">Coba sesuaikan kata kunci atau filter Anda</p>
              </div>
            </div>
          )}
        </AnimatePresence>
            ...
            >
              <form onSubmit={handleSubmit} className="p-8">
                ...
                <div className="space-y-5">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative group">
                      ...
                      <label
                        ...
                        className="flex flex-col items-center justify-center w-full min-h-[160px] bg-[#FAFAF8] border-2 border-dashed border-[#1e293b]/5 rounded-[32px] cursor-pointer hover:bg-white hover:border-[#1e293b]/10 transition-all overflow-hidden relative group"
                      >
                        {uploading ? (
                          <div className="flex flex-col items-center gap-3">
                            <RefreshCw size={24} className="animate-spin text-[#1e293b]/30" />
                            <span className="text-[10px] font-black uppercase tracking-widest text-[#1e293b]/30">Mengunggah...</span>
                          </div>
                        ) : formData.image ? (
                          ...
                          <div className="flex flex-col items-center gap-3 p-8">
                            <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-[#1e293b]/20 shadow-sm">
                              <Plus size={20} />
                            </div>
                            <div className="text-center">
                              <span className="text-[10px] font-black uppercase tracking-widest text-[#1e293b]">Klik untuk Upload Foto</span>
                              <p className="text-[9px] text-[#1e293b]/30 mt-1 uppercase tracking-wider">Format: JPG, PNG, WEBP (Max 5MB)</p>
                            </div>
                          </div>
                        )}
                      </label>
                  ...
                  <button
                    type="submit"
                    className="w-full py-5 bg-[#1e293b] !text-white rounded-[22px] font-black text-[11px] uppercase tracking-[0.3em] shadow-3xl shadow-[#1e293b]/20 hover:scale-[1.02] active:scale-[0.98] transition-all mt-4 select-none"
                  >
        ...
        {confirmDialog && confirmDialog.isOpen && (
          <div className="fixed inset-0 z-[140] flex items-center justify-center p-6">
            ...
            <motion.div
              ...
              className="w-full max-w-md bg-white rounded-[32px] p-6 shadow-2xl relative z-10 border border-[#1e293b]/5 flex flex-col items-center text-center space-y-5"
            >
              {/* Icon */}
              ...
              {/* Title & Description */}
              <div className="space-y-2">
                <h3 className="text-xl font-black text-[#1e293b]" style={{ fontFamily: "var(--font-playfair)" }}>
                  {confirmDialog.title}
                </h3>
              ...
              <div className="flex w-full gap-3 pt-2">
                {confirmDialog.cancelText && (
                  <button
                    ...
                    className="flex-1 py-3.5 bg-[#FAFAF8] text-[#1e293b]/60 border border-[#1e293b]/5 rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] hover:bg-[#F0EDE9] hover:text-[#1e293b] transition-all"
                  >
                    {confirmDialog.cancelText}
                  </button>
                )}
